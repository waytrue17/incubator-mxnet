 ...
 public:
  ...
  void PrePartition(const nnvm::Graph& g,
    ...
    for (auto& kv : options_map) {
      if (kv.first == "static_alloc" || kv.first == "static_shape") { // update static_alloc and static_shape flags
        if(kv.second == "True") {
          options_map_.emplace_back(kv.first, "true");
        } else {
          options_map_.emplace_back(kv.first, "false");
        }
      } else if (kv.first == "param_indices") { // update param_name_set_ for data_indices and param_indices
